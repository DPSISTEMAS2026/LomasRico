...
export class ProductMapperService implements OnModuleInit {
    ...
    async onModuleInit() {
        try {
            ...
            // #region agent log
            fetch('http://127.0.0.1:7828/ingest/0cf486ac-6acc-4365-b51d-aafc32d937ed',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'88a466'},body:JSON.stringify({sessionId:'88a466',runId:'post-fix',hypothesisId:'H-MAPPER',location:'product-mapper.service.ts:onModuleInit',message:'refreshCatalog failed but bootstrap continues',data:{message:message.slice(0,240)},timestamp:Date.now()})}).catch(()=>{});
            // #endregion
            this.logger.warn(`Catalog cache skipped: ${message}`);
        }
    }
